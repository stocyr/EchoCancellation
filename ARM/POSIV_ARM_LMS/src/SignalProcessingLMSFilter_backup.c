...
q15_t CoeffsQ15_new[FILTER_LENGTH];
...
void InitProcessing(void) {
	...
	/* Initialize the FIR module */
	arm_fir_init_q15(&FirStateQ15, FILTER_LENGTH, CoeffsQ15, StateQ15,
			BLOCK_SIZE);
...
#if NUMBER_OF_CHANNELS == 2
void ProcessBlock(uint16_t *Channel1_in, uint16_t *Channel2_in,
		uint16_t *Channel1_out, uint16_t *Channel2_out) {
...
//	/* Copy samples into workbuffer and convert to signed */
//	for (i = 0; i < BLOCK_SIZE; i++) {
//		Y_InBufferQ15[i] = ((q15_t) (Channel1_in[i] - 32768));
//		X_InBufferQ15[i] = ((q15_t) (Channel2_in[i] - 32768));
//	}

	/* Set bit, just for time measurements */
	GPIO_SetBits(GPIOD, GPIO_Pin_0 );

	/* Updating Filtercoefficients */
	for(i = 0;i < NFIR-BLOCK_SIZE; i++){
		x_hist[i] = x_hist[i+BLOCK_SIZE];
	}
	for(i=0; i < BLOCK_SIZE; i++){
		x_hist[i+NFIR-BLOCK_SIZE] = Channel1_in[i];
	}

	for(i = 0; i < NFIR-DELAY; i++){
		summ += w[i]*x_hist[NFIR-DELAY-i-1];
	}
	//arm_dot_prod_q15(w, x_hist, NFIR-DELAY, &y_hat);
	err = Channel2_in[BLOCK_SIZE-1]-summ;

	for (i = 0; i < NFIR-DELAY; i++) {
		w[i] +=  x_hist[NFIR-DELAY-i-1] * u * err;
...
//	/* y_hat = W * X */
//
//	arm_dot_prod_q15(CoeffsQ15, X_InBufferQ15, BLOCK_SIZE, &y_hat);
//	/* e(n) = y(n) - y_hat(n) */
//	err = Y_InBufferQ15[BLOCK_SIZE - 1] - y_hat;
//	/* w = w + u*x(k-deltak:-1:k-NFIR+1)*(err(k)); */
//
//	for (i = 0; i < BLOCK_SIZE; i++) {
//		Wadd[i] = X_InBufferQ15[i] * u * err;
//
//	}
//	//arm_mat_add_q15(CoeffsQ15, Wadd, CoeffsQ15_new);
//
//	for (i = 0; i < BLOCK_SIZE; i++) {
//		CoeffsQ15_new[i] = CoeffsQ15[i]+Wadd[i];
//		CoeffsQ15[i] = CoeffsQ15_new[i];
//	}
//	FirStateQ15.pCoeffs = CoeffsQ15;
//
//	/* Filter one block of samples */
//	arm_fir_q15(&FirStateQ15, Y_InBufferQ15, OutBufferQ15, BLOCK_SIZE);
